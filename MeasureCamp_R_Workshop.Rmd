...
## if you have a score!!!
testData %>% group_by(perc) %>%  summarise(target = mean(clm))
```


### Receiver Operating Characteristic (ROC) Curve

For our claims model we have a predict claim score/probability. Now, if we choose a treshold, say 0.50 and predict: a person will claims if his score is larger than 0.5 and will not claim if his score is smaller than 0.5, we can see on our test data set true positives and false positives. The ROC curve is then created by plotting the true positive rate (TPR) against the false positive rate (FPR) at various threshold values.
