...
If want to analyze data in R, you obviously need data. Until now we have created our data manualy with code. In real life you have already data in files or databases.
...
The functions `group_by` and `summarise` are often used together to aggrgeate data.
...
When training random forest or boosting models, it is often hard to explain the model. I.e. why will the model predict for person X a high claim probablity and why does it predict for person Z a low claim probability?
...
With market basket analysis (also called association rules mining) you can identify from transactional data which products are often bought together. We use a fictional supermarket data set.

From transactional data, in R the data must be converted to a single `transaction object`. You can do this with the package 'arules'.
