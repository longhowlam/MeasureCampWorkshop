...
! WARNING !  **This document will contain more material than I can handle during the 3 hour workshop. **
...
With dplyr you can easily join data frames. There are several joins that are supported. We give a number of examples here.
...
There are several geom _ functions, we will show a few in the code chuncks below.
...
Smoothing lines can be helpfull when you want to see if tehere is a trend in a large scatter plot.
...
We use the restaurants data set again for some plotly barcharts.
...
The plot above sorts the bars based on the kitchen types alphabetically, to sort it on descending number of restautants we need to rorder the factor.
...
Leaflets are interactive maps based on openstreetmaps.
...
The `visnetwork` package can be used to create interactive graph (network) visualisations. You need two data frames, one with the nodes (also called vertices) and one with the edges (links). Let's start with a simple example.
...
Circle packings are a nice way to visualize hierchical data. For example click data or web path logs. First install the `circlepackeR` package from GitHub.
...
I'll also show market basket analysis with the package `arules`. A technique to o analyse transactional data. For example physical purchases in supermarket or web clicks on a site.
...
hist(testData$ensemble_prediction, nclass = 25)
hist(testData$glm_prediction, nclass = 25)
...
explain_cases = bind_rows( 
  trainData %>% filter(exposure > 0.6) %>% slice(1:3),
  trainData %>% filter(exposure < 0.1) %>% slice(1:3)
)
