...
To get a first impression of the data inside a data.frame you can use the functions `summary` or `skimr` (which is in the skimr package that needs to be installed).
...
### write R data frames to a database
...
And also have a look at [this question](https://askubuntu.com/questions/800216/installing-ubuntu-16-04-lts-how-to-install-odbc)
---
...
test = c(
  ...
  "foutje?:  0234XD", "dit is er nog een 1628 EP",
...
popup texts in leaflets can also contain images and animated images. See as an example [this leaflet of Amsterdam](http://www.lhldsd.nl/AmsterdamWijkenOverzichtAnimated.html)
...
Other options can be given via the `visnetwork` function. For example, highlight nearest and node selection:
...
Now that we have the groceries transactions in a transaction object we can calculate rules with the a-priori algorithm.
...
## convert  rules set to a data frame
