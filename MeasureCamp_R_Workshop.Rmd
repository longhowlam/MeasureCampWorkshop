...
```{r, eval = FALSE}
cars_glm = glm(
  clm ~ veh_value + exposure + gender + veh_body + area + veh_age,
  data = trainData,
  family = binomial
)
...
```{r, eval=FALSE}
cars_tree = rpart(clm ~ . -numclaims - claimcst0, data = trainData)
plot(cars_tree)
text(cars_tree)
...
exclude_vars = c("clm", "X_OBSTAT_", "numclaims", "claimcst0" )
inputvars = allvars[ ! allvars %in% exclude_vars]

trainh2o = as.h2o(trainData)
forest = h2o.randomForest(
  x= inputvars,
  y= "clm",
